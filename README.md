...
harbor-svc-queue-mesh is a Kotlin project for backend services. It focuses on this technical goal: Design a Kotlin verification harness for queue systems, covering state machine modeling, transition tables, and failure-oriented tests.
...
The core module exposes a small scoring API. Inputs are simple numeric signals: demand, capacity, latency, risk, and weight. The score uses a threshold of 160, risk penalty 5, latency penalty 4, and weight bonus 3. Tests exercise the public API against the fixture cases in `fixtures/cases.csv`.
